...
description: How I sued AWS API Gateway & Lambda to summarize & track commits.
date: August 28, 2023
...
that would summarize the commit and push it to the repository. The main function, <BackTicks>main</BackTicks>
takes in two arguments, <BackTicks>event</BackTicks> and <BackTicks>context</BackTicks>.
...
def main(request, response):
    ...
    # Create a commit and a detailed commit
    create_commit(...params)
    create_detailed_commit(...params) # can you tell i'm a javascript dev? 

    return 'Success'
```

The anonymous functions <BackTicks>create_commit</BackTicks> and <BackTicks>create_detailed_commit</BackTicks>
are functions that create a commit using the GitHub API, with a GPT summary and the entire commit, editing the 
<BackTicks>logs.md</BackTicks> file and <BackTicks>detailed-logs.md</BackTicks> files accordingly. 
...
def lambda_handler(event, context):
    ...
    create_commit(...params)
    create_detailed_commit(...params)

    return 'Success'
```

Then, with a little bit of fanagling with API Gateway, I had this function running on POST at <BackTicks>api.charliemeyer.xyz/logging-repository</BackTicks>. 
I then pointed the webhook to this URL, and it worked! You can also navigate to [api.charliemeyer.xyz/test](https://api.charliemeyer.xyz/test) to see lambda working in action.
